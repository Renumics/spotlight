...
async def get_layout(request: Request) -> Optional[Dict]:
    ...
    if settings.preconfigured:
        return (app.layout or DEFAULT_LAYOUT).dict(by_alias=True)
...
async def reset_layout(request: Request) -> Dict:
    ...
    if app.data_source and not settings.preconfigured:
        dataset_uid = app.data_source.get_uid()
...
async def set_layout(
    set_layout_request: SetLayoutRequest, request: Request
) -> Optional[Dict]:
    """
    Get layout.
    """
