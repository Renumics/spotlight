...
from .basic_actions import (
    select_points_similaritymap,
    select_points_similaritymap_lasso,
)
from .helpers import (
    get_tab,
...
def test_lasso_selection(
    ...
    """
    test select by lasso in similarity map selects rows
    """
    from ._autogenerated_ui_elements import DataTestTags
    ...
    with screenshot_exception(webdriver):
        ...
        for _ in range(20):
            time.sleep(5)  # wait for backend umap
            select_points_similaritymap_lasso(driver)
            num = int(
                wait_for_tagged_element(
                    webdriver, DataTestTags.DATAGRID_SELECTED_COUNT
                ).text
