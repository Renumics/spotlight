...
def test_import_export_pandas() -> None:
    ...
    weather dataset.
    """
    df = pd.read_csv("data/csv/australian-weather-small.csv")
    # df["WindDir9am_category"] = df["WindDir9am"].astype("category")
    df["Date"] = pd.to_datetime(df["Date"])
    df["MissedDate"] = df["Date"]
    df.loc[
        np.random.choice(len(df), len(df) // 4, replace=False), "MissedDate"
    ] = pd.NaT
    for name, dtype in df.dtypes.items():
        if dtype is np.dtype(object):
            df[name].fillna("", inplace=True)
    df["SureRainToday"] = [x == "Yes" for x in df["RainToday"]]
    df["SureRainTomorrow"] = [x == "Yes" for x in df["RainTomorrow"]]
    with tempfile.TemporaryDirectory() as output_folder:
        output_h5_file = os.path.join(output_folder, "dataset.h5")
        with Dataset(output_h5_file, "w") as dataset:
...
def test_import_pandas_with_dtype() -> None:
    ...
    for column in ("RainToday", "RainTomorrow"):
        true_mask = df[column] == "Yes"
        df[column] = 0
        df.loc[true_mask, column] = 1
        dtype[column] = bool
    df["Mesh"] = np.random.choice(
        ["data/meshes/bunny.off", None], len(df), p=[0.05, 0.95]  # type: ignore
    )
    dtype["Mesh"] = Mesh
    df["Image"] = np.random.choice(
        ["data/images/lena.ico", None], len(df), p=[0.05, 0.95]  # type: ignore
    )
    dtype["Image"] = Image
    df["Audio"] = np.random.choice(
        ["data/audio/1.wav", None], len(df), p=[0.05, 0.95]  # type: ignore
    )
    dtype["Audio"] = Audio
...
def test_import_csv() -> None:
    ...
    with tempfile.TemporaryDirectory() as output_folder:
        output_h5_file = os.path.join(output_folder, "dataset.h5")
        with Dataset(output_h5_file, "w") as dataset:
            dataset.from_csv("data/csv/australian-weather-small-edited.csv")
            assert set(dataset.keys()) == set(
                pd.read_csv(
                    "data/csv/australian-weather-small-edited.csv", nrows=1
                ).columns
            )
            assert dataset["RainToday", 0] == ""
            assert dataset["RainToday", 1] == ""
...
def test_import_csv_with_dtype() -> None:
    ...
    with tempfile.TemporaryDirectory() as output_folder:
        ...
        for folder_name in ("audio", "images", "meshes", "video"):
            shutil.copytree(
        ...
        df["video"] = df["video1"] = df["video2"] = np.random.choice(
            list(
                filter(
                    os.path.isfile,
                    glob(
                        f"{os.path.join(output_folder, 'video.copy')}/**",
                        recursive=True,
                    ),
                )
            ),
